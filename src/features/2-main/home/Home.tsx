...
export const Home = () => {
  const [posImg, setPosImg] = useState({x: 50, y: 45});
  ...
  const handleMouseMove = (e: MouseEvent<HTMLDivElement>) => {
    ...
    const speedImg = 1.2
    setPosImg({x: 50 + posX * speedImg, y: 45 - posY * speedImg});
  ...
  return (
    <div className={s.containerHome} onMouseMove={throttledFunction }>
      <div className={`${s.background}`}>
        ...
        <div className={`${s.item} containerApp`} style={{transform: `translate(${posItem.x}px, ${posItem.y}px)`}}>
          <h1>Welcome!</h1>
